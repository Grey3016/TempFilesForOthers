...
class Decoder3d(nn.Module):
    ...
    def __init__(
        ...
        out_dim = dims[0]
        self.head = nn.Sequential(
            CausalConv3d(z_dim, out_dim, 3, padding=1),
            nn.SiLU(),
            RMS_norm(out_dim, images=False),
        )
        ...
        # output block
        self.conv1 = CausalConv3d(dims[-1], 12, 3, padding=1)
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0], first_chunk=False):
        ...
        for layer in self.upsamples:
            if feat_cache is not None:
                x = layer(x, feat_cache, feat_idx, first_chunk=first_chunk)
            else:
                x = layer(x)
    ...
    def encode(self, video):
        try:
            if not isinstance(video, list):
                raise TypeError("video should be a list")
...
class VAEModel(nn.Module):
    ...
    def encode(self, x, scale=0.18215, feat_cache=None, feat_idx=[0]):
        z = self.encoder(x, feat_cache, feat_idx)
        return z * scale

    def decode(self, z, scale=0.18215, feat_cache=None, feat_idx=[0], first_chunk=False):
        z = z / scale
        out = self.decoder(z, feat_cache, feat_idx, first_chunk)
        return out