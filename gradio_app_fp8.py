...
# Assuming save_video is imported from here and has been modified
...
# --- Custom Local Temp Folder Setup ---
# Define the local Temp folder path relative to the project directory
LOCAL_TEMP_DIR = 'Temp'
# Ensure the directory exists
os.makedirs(LOCAL_TEMP_DIR, exist_ok=True)
print(f"Using local temporary directory: {os.path.abspath(LOCAL_TEMP_DIR)}")
# ------------------------------------
...
def generate_video(
    ...
    # Set the sample rate constant
    SAMPLE_RATE = 16000
    
    # Define the local path for the intermediate WAV file
    # This path will be used by the modified save_video to bypass Appdata/Temp
    wav_filename = f"ovi_audio_{uuid.uuid4().hex[:8]}.wav"
    local_wav_path = os.path.join(LOCAL_TEMP_DIR, wav_filename)

    # Define the local path for the final MP4 video
    mp4_filename = f"ovi_video_{uuid.uuid4().hex[:8]}.mp4"
    output_path = os.path.join(LOCAL_TEMP_DIR, mp4_filename)

    try:
        image_path = image if image is not None else None
        ...
        # Pass the generated audio data and the local path for the temporary WAV file.
        # The modified save_video function will handle the writing and cleanup of local_wav_path.
        save_video(
            output_path, 
            generated_video, 
            audio_data=generated_audio, 
            temp_audio_path=local_wav_path,
            fps=24, 
...
def generate_image(text_prompt, image_seed, image_height, image_width):
    ...
    # Save image to the local Temp folder for consistency
    image_filename = f"ovi_image_{uuid.uuid4().hex[:8]}.png"
    image_path = os.path.join(LOCAL_TEMP_DIR, image_filename)
